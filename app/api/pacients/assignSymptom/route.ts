...
const bodySchema = z.object({
  ...
  symptomIds: z.array(z.number()).min(1),  // Una o más enfermedades
})

export async function POST(req: Request) {
  try {
    const json = await req.json()
    const { pacientId, symptomIds } = bodySchema.parse(json)
    ...
    const records = symptomIds.map((symptomId) => ({
      pacienteId: pacientId,
      sintomaId: symptomId,
    }))

    // Inserta en la tabla pivote, evitando duplicados
    await db.sintomaPaciente.createMany({
      data: records,
      skipDuplicates: true,
    })
    ...
    console.error("Error assigning symptoms to pacient:", error)
    return new Response(
